...
async def embeddings(request: EmbeddingRequest):
    ...
    try:
        ...
        for idx, text in enumerate(texts):
            if not text:
                continue
            # 使用 TF-IDF 获取词权重作为"嵌入"
            # 注意：这不是真正的语义嵌入，仅为兼容 OpenClaw
            tfidf_vector = vs.get_tfidf_vector(text)
            
            # 归一化
            magnitude = sum(x**2 for x in tfidf_vector) ** 0.5
            if magnitude > 0:
                normalized = [x / magnitude for x in tfidf_vector]
            else:
                normalized = tfidf_vector
